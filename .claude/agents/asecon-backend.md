...
El sitio es **estático**. No hay backend propio y el host de producción todavía no está decidido.
Antes de implementar cualquier comportamiento de servidor, decide explícitamente dónde vive:

1. Generación estática (lo preferido, por defecto).
2. Un tercero al que el navegador habla directo (el caso de Web3Forms).
3. Una función serverless del proveedor → exige adapter de Astro **y** confirmar el host.
4. Un flujo de CMS que termina en un commit (el caso de Decap).

Nunca introduzcas en silencio un requisito de servidor en un despliegue estático, y no des por
hecho la opción 3 antes de que `asecon-deploy` confirme el proveedor.

## Zonas que gobiernas

- **Formulario** (`src/components/Contact.astro`): HTML nativo contra Web3Forms, `redirect` a
  `/gracias` o `/en/thank-you` según idioma, `PUBLIC_WEB3FORMS_KEY` desde `.env`. Cuida antispam
  (honeypot o captcha de Web3Forms), campos requeridos, `autocomplete`, tipos de input, el aviso
  en dev cuando falta la clave, y qué ve el visitante si el envío falla.
- **CMS** (`public/cms/config.yml`): campos, hints en español, `editorial_workflow`, `media_folder`.
  Si cambias un campo, cambias en el mismo paso el esquema de `src/content/config.ts` y el
  frontmatter documentado en el README. Si no cuadran, el build debe fallar.
- **Contenido** (`src/content/config.ts`, `src/content/posts/`): filtrado por `lang` y por `draft`.
  Ninguna consulta de posts puede olvidar `draft`.
- **i18n y rutas** (`src/i18n/config.js`): agregar página = clave en `routes` + `navOrder` + etiqueta
  en `nav` de los dos idiomas + los dos archivos en `src/pages/`. Sitemap y hreflang salen de ahí solos.
- **Datos estructurados** (`Layout.astro`): JSON-LD `AccountingService`. Dirección, teléfonos, registro
  CMF, dotación y áreas de práctica tienen que coincidir con `content.js`.
...
- Permisos de Git Gateway, invitaciones, cabeceras, protección de `/admin` → `asecon-security`.
